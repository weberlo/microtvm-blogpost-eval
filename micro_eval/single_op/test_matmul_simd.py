...
def matmul_arm_micro_template(A, B, out_dtype, tens_config):
    ...
    if tens_config == TensorizeConfig.NONE:
        pass
    elif tens_config == TensorizeConfig.SMALL:
        x, y = C.op.axis
        z, = C.op.reduce_axis

        xo, xi = sched[C].split(x, factor=1)
        yo, yi = sched[C].split(y, factor=1)
...
def main():
    ...
    with micro.Session(DEV_CONFIG) as sess:
        ...
        small_simd_sched, small_simd_arg_bufs = matmul_arm_micro_template(A_TENSOR, B_TENSOR, OUT_DTYPE, TensorizeConfig.SMALL)
        small_simd_time = eval_micro(sess, small_simd_sched, small_simd_arg_bufs)
        small_simd_time -= time_overhead

        large_simd_sched, large_simd_arg_bufs = matmul_arm_micro_template(A_TENSOR, B_TENSOR, OUT_DTYPE, TensorizeConfig.LARGE)
