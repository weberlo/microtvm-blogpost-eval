...
from topi.arm_cpu.cortex_m7.conv2d.direct import conv2d_direct
from topi.arm_cpu.cortex_m7.conv2d.direct_simd import conv2d_direct_simd
...
DATA_TYPE = LabelledShape(N=N, C=CI, H=H, W=W, dtype=IN_DTYPE)
KERNEL_TYPE = LabelledShape(H=KH, W=KW, I=CI, O=CO, dtype=IN_DTYPE)
OUT_TYPE = LabelledShape(N=N, C=CO, H=H, W=W, dtype=OUT_DTYPE)

# CMSIS out dtype is the same as in dtype
CMSIS_OUT_TYPE = LabelledShape(N=N, C=CO, H=H, W=W, dtype=IN_DTYPE)
CMSIS_BIAS_LSHIFT = 0
# CMSIS_OUT_RSHIFT = 9  # original param
CMSIS_OUT_RSHIFT = 0

BIAS_TYPE = LabelledShape(B=CO, dtype=IN_DTYPE)
...
def eval_micro(
        sess, c_mod,
        data_nt: LabelledTensor,
        kernel_nt: LabelledTensor,
        out_type: LabelledShape,
        lib_src_paths=None):
    ...
    micro_mod = create_micro_mod(
        c_mod, DEV_CONFIG, lib_src_paths=lib_src_paths, lib_headers=CMSIS_HEADERS,
    ...
    check_conv2d_output(
        data_nt,
        kernel_nt,
        LabelledTensor(output_np, out_type),
        STRIDES, PADDING)
...
#    ]
    CMSIS_CONV_SRC_PATH = f'{os.path.dirname(__file__)}/../../../cmsis_src/cmsis_rgb_conv2d.c'
    CMSIS_SRC_PATHS = [
        f'{CMSIS_NN_PATH}/CMSIS/NN/Source/NNSupportFunctions/arm_q7_to_q15_reordered_no_shift.c',
    ...
    print('out type', out_nt.data.shape)
...
#    micro_func = micro_mod['arm_fast_conv2d_wrapper']
    ...
    print('out type', output_tvm.shape)
    ...
    for data_layout, kernel_layout in [('NCHW', 'OIHW'), ('NHWC', 'HWIO')]:
        ...
        with TARGET:
            sched, arg_bufs = conv2d_direct(
                data_nt.shape.serialize(), kernel_nt.shape.serialize(),
